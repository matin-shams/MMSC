...
paths = [
    ...
    ("Classical (IE)", f"output/classical_ie/{data_name}.txt", "--"),
    ("Boris (IE)", f"output/boris_ie/{data_name}.txt", "--"),
]

plt.figure()
for label, path, linestyle in paths:
    try:
        with open(path) as f:
            data = [float(line.strip()) for line in f if line.strip()]
        timesteps = np.arange(len(data)) * dt
        plt.plot(timesteps, data, label=label, linestyle=linestyle)
